...
class MyTest(unittest.TestCase):
    def test(self):
        ...
        p.append(values.decayvalues['p'])
        theta.append(values.decayvalues['theta'])
        En.append(values.decayvalues['E'])

        px = p * np.cos(theta)
        py = p * np.sin(theta)

        self.assertEqual(round(sum(px[:-1]), 5), round(px[-1], 5))
        self.assertEqual(round(sum(py[:-1]), 5), round(py[-1], 5))
        self.assertEqual(round(sum(En[:-1]), 5), round(En[-1], 5))
