...
class Lab2ColCalc(nbodycollision.LabCalc):
    ...
    def __init__(self,particles,momenta,angles,masses):
        self._inc_particles = particles                                         #name of incoming particles (array of 2 strings)
        inc_energies = self._set_energies(momenta,masses)                       #energy of both particles (array of 2 floats)
        # From here on, all calculations are for outgoing particle ('virtual')
        self._pExyz = self._set_pExyz(momenta,angles,inc_energies)              #4-momentum of the resultant particle (dictionary of 4 floats)
        self._p = self._set_p()                                                 #module of the 3-momentum of the resultant particle (float)
        self._mass = self._set_mass(self._p,self._pExyz['E'])                   #virtual mass of the resultant particle (float)
        self._theta = self._set_angle(self._pExyz['x'],self._pExyz['y'])        #angle of the resultant particle (float)

        self._values = self._set_values()                                       #dictionary with E, p, theta, mass
